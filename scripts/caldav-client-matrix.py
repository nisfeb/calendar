...
def names(base, jar, cal):
    ...
    # an outage is a failure, never a list a membership check could pass on
    if not isinstance(rows, list): sys.exit('events.json failed on %s: %s' % (base, str(rows)[:60]))
...
def wait(fn, secs=60, nudge=False):
    # a push lands on its own; a pull needs the follower's pass, so a wait
    # for one nudges it every 15 s. A loaded machine is slow, not wrong.
    t0 = last = time.time()
    while time.time() - t0 < secs:
        if nudge and time.time() - last > 15:
            curl(FJ, FOL + '/apps/calendar/caldav/sync', '{}', 'POST'); last = time.time()
        try:
            if fn(): return True
...
seeded = not names(SRC, SJ, 'default')
if seeded:
...
try:
    check('initial pull matches the source', wait(lambda: names(FOL, FJ, cid) == names(SRC, SJ, 'default'), 90, True), (names(FOL, FJ, cid), names(SRC, SJ, 'default')))
    before = len(names(SRC, SJ, 'default'))
    # follower -> source
    poke(FOL, FJ, {'action': 'add-event', 'cal': cid, 'cat': 'timed', 'kind': 'once', 'start_ms': 1792000000000, 'fin': 'dur', 'dur_min': 30, 'meta': {'name': 'cm from follower', 'tags': ['crossed']}})
    check('follower add reaches the source', wait(lambda: 'cm from follower' in names(SRC, SJ, 'default')), names(SRC, SJ, 'default'))
    u = uid_of(FOL, FJ, 'cm from follower')
    poke(FOL, FJ, {'action': 'edit-event', 'id': u, 'cat': 'timed', 'kind': 'once', 'start_ms': 1792000000000, 'fin': 'dur', 'dur_min': 30, 'meta': {'name': 'cm from follower edited', 'tags': ['crossed']}})
    check('follower edit reaches the source', wait(lambda: 'cm from follower edited' in names(SRC, SJ, 'default')))
    ...
    poke(SRC, SJ, {'action': 'add-event', 'cat': 'timed', 'kind': 'once', 'start_ms': 1792003600000, 'fin': 'dur', 'dur_min': 30, 'meta': {'name': 'cm from source'}})
    check('source add reaches the follower', wait(lambda: 'cm from source' in names(FOL, FJ, cid), 90, True), names(FOL, FJ, cid))
    fn = names(FOL, FJ, cid)
    ...
    # deletes both ways
    poke(FOL, FJ, {'action': 'del-event', 'id': u})
    check('follower delete reaches the source', wait(lambda: 'cm from follower edited' not in names(SRC, SJ, 'default')))
    poke(SRC, SJ, {'action': 'del-event', 'id': uid_of(SRC, SJ, 'cm from source')}); poke(SRC, SJ, {'action': 'del-event', 'id': 'cm-weekly@test'})
    check('source deletes reach the follower', wait(lambda: not [n for n in names(FOL, FJ, cid) if n.startswith('cm ') and n != 'cm seed'], 90, True), names(FOL, FJ, cid))
    ...
    poke(SRC, SJ, {'action': 'add-event', 'cat': 'timed', 'kind': 'once', 'start_ms': 1792100000000, 'fin': 'dur', 'dur_min': 30, 'meta': {'name': 'cm clash'}})
    check('a source add to clash on reaches the follower', wait(lambda: 'cm clash' in names(FOL, FJ, cid), 90, True))
    ...
    poke(FOL, FJ, {'action': 'edit-event', 'id': cu, 'cat': 'timed', 'kind': 'once', 'start_ms': 1792100000000, 'fin': 'dur', 'dur_min': 30, 'meta': {'name': 'cm clash (follower)'}})
    check('412 on push is logged as a conflict', wait(lambda: any(c['uid'] == cu for c in curl(FJ, FOL + '/apps/calendar/google/conflicts.json'))), curl(FJ, FOL + '/apps/calendar/google/conflicts.json'))
    check("the source's version wins on the follower", wait(lambda: 'cm clash (source)' in names(FOL, FJ, cid), 90, True), names(FOL, FJ, cid))
    poke(SRC, SJ, {'action': 'del-event', 'id': cu}); time.sleep(2); prod()
    curl(FJ, FOL + '/apps/calendar/google/conflicts/clear', '{}', 'POST')
finally:
    # a crash above still unfollows, revokes and removes the seed
    # cleanup
    ...
    if seeded: poke(SRC, SJ, {'action': 'del-event', 'id': uid_of(SRC, SJ, 'cm seed')})
    # only what this run made: the follower may follow other calendars
check('cleanup', cid not in [c['id'] for c in curl(FJ, FOL + '/apps/calendar/calendars.json')])
